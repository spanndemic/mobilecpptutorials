...
Now we need to add our C++ source files to the project, including the header file generated by Djinni, and both of our hand-coded implementation files. At this point, our project’s folder structure should look like this:
...
If the resulting dialog, be sure to select ‘Create folder references’ and not ‘Copy items if needed’... otherwise Xcode will create duplicates of the files inside of the project folder, which would then be out of sync with the C++ code in our `src` folder. Also, be sure you check the box next to your deployment target so the files will be included when you publish:
...
Our last step is to replace the contents of `main.cpp` with the following:
